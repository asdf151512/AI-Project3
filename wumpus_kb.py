...
def axiom_generator_percept_sentence(t, tvec):
    ...
    if not tvec[1]:
        axiom_str += '~' 
    axiom_str += percept_breeze_str(t)
    axiom_str += ' & ' 
    if not tvec[2]:
        axiom_str += '~' 
    axiom_str += percept_glitter_str(t)
    axiom_str += ' & ' 
    if not tvec[3]:
        axiom_str += '~' 
    axiom_str += percept_bump_str(t)
    axiom_str += ' & ' 
    if not tvec[4]:
        axiom_str += '~' 
    axiom_str += percept_scream_str(t)
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    #Just generate no pit and wumpus in the initial location
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    """
    Generate the relation with pits and breezes
    """
    pits = []
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    """
    Generate the relation with wumpus and stench remember that the same location is possible in the world KB
    """
    wumpus = []
    for (xVal,yVal) in [((x-1),y),(x,(y-1)),((x+1),y),(x,(y+1))]:
        if xVal >= xmin and xVal <= xmax and yVal >= ymin and yVal <= ymax:
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    """
    This is simple just output all location and OR between each of them
    """

    locations = [wumpus_str(xVal,yVal) for xVal in range(xmin,xmax + 1) for yVal in range(ymin,ymax + 1)]
    axiom_str = ' | '.join(locations)
    #print axiom_str
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    """
    This is a little hard since we have to output all implication as below:
    Wumpus in one location would imply no Wumpus in any other location
    """
    locations = [wumpus_str(xVal,yVal) for xVal in range(xmin,xmax + 1) for yVal in range(ymin,ymax + 1)]
    axiom=[]
    for location in locations:
        not_location=[]
        #record the location that no wumpus
        for temp_location in locations:
            if temp_location!=location:
                not_location.append('~'+temp_location)
        #record the implication
        axiom.append('({0} >> ({1}))'.format(location,' & '.join(not_location)))
    axiom_str = '{0}'.format(' & '.join(axiom))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    #print axiom_str
    return axiom_str
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    "*** YOUR CODE HERE ***"
    """
    use state_loc_str(x,y,t) to do
    """
    locations = [state_loc_str(xVal,yVal,t) for xVal in range(xmin,xmax + 1) for yVal in range(ymin,ymax + 1)]
    not_location=[]
    #generate the list of not location
    for location in locations:
        if location!=state_loc_str(xi,yi,t):
            not_location.append('~'+location)
    #combine now location and not location
    axiom_str = '{0} & {1}'.format(state_loc_str(xi,yi,t),' & '.join(not_location))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    #print axiom_str
    return axiom_str
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    "*** YOUR CODE HERE ***"
    """
    use imply to show this case
    """
    
    # Comment or delete the next line once this function has been implemented.
    states=[state_heading_north_str(t),state_heading_east_str(t),state_heading_south_str(t),state_heading_west_str(t)]
    directions=['North','East','South','West']
    current_direction=''
    for direction in directions:
        if direction.upper()==heading.upper():
            current_direction = 'Heading{0}{1}'.format(direction,t)
            break;
    #print current_direction
    not_directions=[]
    for state in states:
        if state!=current_direction:
            not_directions.append('~'+state)
    axiom_str = '{0} & {1}'.format(current_direction,' & '.join(not_directions))
    print axiom_str
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    "*** YOUR CODE HERE ***"
    """
    use state_have_arrow_str(t) and state_wumpus_alive_str(t)
    """
...
def axiom_generator_location_OK(x, y, t):
    ...
    "*** YOUR CODE HERE ***"
    """
    use state_OK_str(x,y,t) and pit_str(x,y) and wumpus_str(x,y) and state_wumpus_alive_str(t)
    """
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    "*** YOUR CODE HERE ***"
    """
    When we're at location(x,y) at time t while percept the breeze, it means that this location is breezy
    """
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    "*** YOUR CODE HERE ***"
    """
    When we're at location(x,y) at time t while percept the stench, it means that this location has a stench
    """
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    moves = ['({0} & (~{1} | {2} | {3} | {4} | {5} | {6} | {7}))'.format(state_loc_str(x,y,t),action_forward_str(t),percept_bump_str(t+1),action_grab_str(t),action_shoot_str(t),action_turn_left_str(t),action_turn_right_str(t),action_climb_str(t))]

    for ((xVal,yVal),move) in [(((x-1),y),'E'),((x,(y-1)),'N'),(((x+1),y),'W'),((x,(y+1)),'S')]:
        if xVal >= xmin and xVal <= xmax and yVal >= ymin and yVal <= ymax:
            if move == 'N':
                moves.append('({0} & ({1} & {2} & ~{3}))'.format(state_loc_str(x,y-1,t),state_heading_north_str(t),action_forward_str(t),percept_bump_str(t+1)))
            if move == 'E':
                moves.append('({0} & ({1} & {2} & ~{3}))'.format(state_loc_str(x - 1, y, t), state_heading_east_str(t), action_forward_str(t),percept_bump_str(t+1)))
            if move == 'W':
                moves.append('({0} & ({1} & {2} & ~{3}))'.format(state_loc_str(x + 1, y, t), state_heading_west_str(t), action_forward_str(t),percept_bump_str(t+1)))
            if move == 'S':
                moves.append('({0} & ({1} & {2} & ~{3}))'.format(state_loc_str(x,y+1,t),state_heading_south_str(t),action_forward_str(t),percept_bump_str(t+1)))
...
def axiom_generator_have_arrow_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    """
    if I still have arrow at time t+1, it means that :
    I have arrow at time t and I didn't shoot it at time t
    """
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    """
    If wumpus still alive at time t+1, it means that:
    wumpus alive at time t and we didn't perceive the scream at time t+1
    """
...
def axiom_generator_heading_north_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    """
    If I  heading north at time t+1, it means that:
    either:
    heading north at time t and no turn left or turn right at time t
    heading west at time t and turn right at time t
    heading east at time t and turn left at time t
    """
    # Comment or delete the next line once this function has been implemented.
    action = []
    action.append('({0} & (~{1} & ~{2}))'.format(state_heading_north_str(t),action_turn_left_str(t),action_turn_right_str(t)))
    action.append('({0} & {1})'.format(state_heading_west_str(t),action_turn_right_str(t)))
    action.append('({0} & {1})'.format(state_heading_east_str(t),action_turn_left_str(t)))
    axiom_str = '{0} <=> ({1})'.format(state_heading_north_str(t+1),' | '.join(action))
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    """
    If I  heading east at time t+1, it means that:
    either:
    heading east at time t and no turn left or turn right at time t
    heading north at time t and turn right at time t
    heading south at time t and turn left at time t
    """
    action = []
    action.append('({0} & (~{1} & ~{2}))'.format(state_heading_east_str(t),action_turn_left_str(t),action_turn_right_str(t)))
    action.append('({0} & {1})'.format(state_heading_north_str(t),action_turn_right_str(t)))
    action.append('({0} & {1})'.format(state_heading_south_str(t),action_turn_left_str(t)))
    axiom_str = '{0} <=> ({1})'.format(state_heading_east_str(t+1),' | '.join(action))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    """
    If I  heading south at time t+1, it means that:
    either:
    heading south at time t and no turn left or turn right at time t
    heading east at time t and turn right at time t
    heading west at time t and turn left at time t
    """
    action = []
    action.append('({0} & (~{1} & ~{2}))'.format(state_heading_south_str(t),action_turn_left_str(t),action_turn_right_str(t)))
    action.append('({0} & {1})'.format(state_heading_east_str(t),action_turn_right_str(t)))
    action.append('({0} & {1})'.format(state_heading_west_str(t),action_turn_left_str(t)))
    axiom_str = '{0} <=> ({1})'.format(state_heading_south_str(t+1),' | '.join(action))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    """
    If I  heading west at time t+1, it means that:
    either:
    heading west at time t and no turn left nor turn right at time t
    heading south at time t and turn right at time t
    heading north at time t and turn left at time t
    """
    action = []
    action.append('({0} & (~{1} & ~{2}))'.format(state_heading_west_str(t),action_turn_left_str(t),action_turn_right_str(t)))
    action.append('({0} & {1})'.format(state_heading_south_str(t),action_turn_right_str(t)))
    action.append('({0} & {1})'.format(state_heading_north_str(t),action_turn_left_str(t)))
    axiom_str = '{0} <=> ({1})'.format(state_heading_west_str(t+1),' | '.join(action))
    #print axiom_str
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_only_north(t):
    ...
    "*** YOUR CODE HERE ***"
    # Comment or delete the next line once this function has been implemented.
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_north_str(t),state_heading_east_str(t),state_heading_west_str(t),state_heading_south_str(t))
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_only_east(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_east_str(t),state_heading_north_str(t),state_heading_west_str(t),state_heading_south_str(t))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_only_south(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_south_str(t),state_heading_east_str(t),state_heading_west_str(t),state_heading_north_str(t))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_heading_only_west(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_west_str(t),state_heading_east_str(t),state_heading_north_str(t),state_heading_south_str(t))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    actions = [action_forward_str(t),action_grab_str(t),action_shoot_str(t),action_climb_str(t),action_turn_left_str(t),action_turn_right_str(t),action_wait_str(t)]
    clauses = []
    for action in actions:
        no_actions = ['~'+act for act in actions if act!=action]
        clauses.append('({0} <=> ({1}))'.format(action,' & '.join(no_actions)))
